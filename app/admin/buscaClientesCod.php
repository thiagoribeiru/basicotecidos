...
if (isset($_GET) and $_GET['id']!='') {
    ...
    $cli_query = $sql->query("select * from usuarios where ativo = 1 and nivel = 1 and id = ".$_GET['id']) or die (mysqli_error($sql));
    if (mysqli_num_rows($cli_query)==1) {
        ...
        exit;
    } else if (mysqli_num_rows($cli_query)==0) {
        $retorno['error'] = 1;
